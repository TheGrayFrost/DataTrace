...
pair<ADDRINT,ADDRINT>  addr_bss;
pair<ADDRINT,ADDRINT>  addr_data;
...
map<THREADID,list<struct accesslocks> > mp;
double dataarray[NTHREADS][9];
double dataarray2[NTHREADS][7];
double dataarray3[NTHREADS][9];
double dataarray4[NTHREADS][6];
std::set<ADDRINT>uniqmutex;
std::set<ADDRINT>uniqbar;
std::set<ADDRINT>uniqsem;
std::set<ADDRINT>uniq_rw;
...
int varcount=1;
int mutcount=1;
string program_name; //for storing name of executable file
...
//dataarry4
std::list<struct sequential> sequential_list;
std::list<struct sequentialthreadcount> sequentialthreadlist;
std::map<THREADID,std::vector<ADDRINT> >globalvarthreadmap;
////////
...
//char * name;
	...
	PIN_GetLock(&lock, tid+1);
       ...
       // cout<<'\n';
         tr1.name=rtnName;
         tr1.tid=0;
         tr1.mutex_var=0; tr1.sem_var=addr; tr1.rw_var=0;
...
//char * name;
	...
	PIN_GetLock(&lock, tid+1);
       ...
       // cout<<'\n';
         tr1.name=rtnName;
         tr1.tid=0;
         tr1.mutex_var=0; tr1.sem_var=0; tr1.rw_var=addr;
	   tr1.global_var=00000;
        tr1.cond_var=00000;
        tr1.node_type=1;
...
MutexFile << "  Rtn: " << setw(8) << hex << RTN_Address(rtn) << " " << RTN_Name(rtn) << endl;
            string path;
            INT32 line;
            ...
            if (path != "")
            {
                MutexFile << "File " << path << " Line " << line << endl; 
            }
            ...
            {
               MutexFile << "    " << setw(8) << hex << INS_Address(ins) << " " << INS_Disassemble(ins) << endl;
                if (INS_Valid(lastIns))
                {
                    ...
                    { 
                        MutexFile << "  rtnInternalRangeList.push_back " << setw(8) << hex << rtnInternalRange.start << " " << setw(8) << hex << rtnInternalRange.end << endl;
                        ...
                        for (vector<RTN_INTERNAL_RANGE>::iterator ri = rtnInternalRangeList.begin(); ri != rtnInternalRangeList.end(); ri++)
                        {
                            ...
                            {
                                MutexFile << "***Error - above instruction already appeared in this RTN\n";
                               MutexFile << "  in rtnInternalRangeList " << setw(8) << hex << ri->start << " " << setw(8) << hex << ri->end << endl;
                                exit (1);
                            }
                        }
...
ADDRINT addr1=reinterpret_cast<ADDRINT>(ip);
//ADDRINT addr2=reinterpret_cast<ADDRINT>(addr);
//INS  ins1= ins;
//MutexFile << addr1 << "IP" << "Address" << addr2  << endl;

 if((address >= addr_bss.first && address <= addr_bss.second)||(address >= addr_data.first && address <= addr_data.second))
	{

std::string ins_str = str_of_ins_at[addr1];
std::string check = "rip";
MutexFile << " RiP SEE " << ins_str.find(check) << endl;	
//if (ins_str.find(check) > 0 && ins_str.find(check) < 450)
...
//std::string check = "r";
//MutexFile << " RIP SEE " << ins_str.find(check) << endl;
MutexFile << " Disassembly " <<  ins_str  << " ip " << addr1 << " Address " << address <<endl;
...
//RTN rtn1 = INS_Rtn(ins);
 //cout << " Size of Routines in bytes" << INS_Rtn(ins) << endl;
 //RTN_Open(rtn1);
//cout << RTN_Address(rtn1) << endl;

// RTN_Close(rtn1);
//MutexFile<< " CHECK " << " Address "  << " rtn " << RTN_Name(rtn1).c_str() << endl; 
//MutexFile<< " CHECK " << " Address " << INS_Address(ins) << " rtn " << INS_Rtn(ins)<< endl; 
		...
		if(IMG_IsMainExecutable(Img))
		{
			...
			{ 
                              ...
                              //  cout << SEC_Name(sec) <<'\n';
                                ...
                                found2=SEC_Name(sec).compare(".data");
				...
				if(found==0)
				{
					...
					highaddr=lowaddr+secsize*4;
					std::cout << "High addr: " << highaddr << '\n';
					//OutFile << "High addr: " << highaddr << '\n';
					//printf("%lu\n",highaddr);
				
						addr_bss=make_pair(lowaddr,highaddr);
                   }			
					...
					//printf("%lu\n",highaddr);
						...
						addr_data=make_pair(lowaddr,highaddr);
                   }			
...
double global_updates_thread(THREADID tid)
{
  ...
  std::map<int,std::list<struct globalregion> >::iterator it;
  std::list<struct globalregion>::iterator l;

  set<ADDRINT> lt;
  set<ADDRINT>::iterator tt;
                                          ...
                                          if(l->tid==tid)
                                          {
                                                ...
                                                { 
                                                      ...
                                                      if (l->global_var == *itg)
                                                      {
                                                            ...
                                                            if(gbr.find(tid)==gbr.end())
                                                            {
                                                               lt.insert(l->global_var);
                                                               gbr[tid]=lt;
                                                              // access_count++;
                                                            }
                                                            else
                           ...
                           access_count=gbr[tid].size();
   
  // cout << "ACCESS COUNTTTTTTTTTTT" << access_count;
     if(access_count==0)
return 0.05;
else
return (access_count);
...
void countuniqueaddr()
{
    std::map<int,std::list<struct globalregion> >::iterator itgr;
    
    //cout<<globalmap.size()<<"  GGGGGGGGGGGGGGGGD"<<endl;
    for(itgr=globalmap.begin();itgr!=globalmap.end();itgr++)
    {

      std::list<struct globalregion>::iterator it;
        
        for(it=itgr->second.begin();it!=itgr->second.end(); ++it)
        {
         
            if(it->sync=="MUTEX"&&it->mutex_var)
              uniqmutex.insert(it->mutex_var);
            else if(it->sync=="BARRIER"&&it->mutex_var)
              uniqbar.insert(it->mutex_var);
            
             if(it->rw_var)
              uniq_rw.insert(it->rw_var);

            if(it->sem_var)
              uniqsem.insert(it->sem_var);
        }
    }

}

double count_uniq_mut()
{
    if(uniqmutex.size()==0)
      return 0.05;

  return uniqmutex.size();
}

double count_uniq_barr()
{
  if(uniqbar.size()==0)
      return 0.05;
  return uniqbar.size();
...
double Mutexcountperthread(THREADID thr_id)
{
	std::map<int,std::list<struct globalregion> >::iterator itgr;
	...
	for(itgr=globalmap.begin();itgr!=globalmap.end();itgr++)
	{
		std::list<struct globalregion>::iterator it;
			...
			for(it=itgr->second.begin();it!=itgr->second.end(); it++)
			{
				if(it->tid==thr_id)
				{
					std::list<ADDRINT>::iterator gt;

					for(gt=glist.begin();gt!=glist.end();gt++)
					{
						if(it->global_var==*gt)
						{
							if(it->sync=="MUTEX")
...
double semcountperthread(THREADID thr_id)
{
	std::map<int,std::list<struct globalregion> >::iterator itgr;
	//countaccess obj;
...
double barcountperthread(THREADID thr_id)
{
	std::map<int,std::list<struct globalregion> >::iterator itgr;
	//countaccess obj;
...
double rwlockperthread(THREADID thr_id)
{
	std::map<int,std::list<struct globalregion> >::iterator itgr;
	//countaccess obj;
...
double unsynccountperthread(THREADID thr_id)
{
	std::map<int,std::list<struct globalregion> >::iterator itgr;
	//countaccess obj;
...
/////////////////functions trials of each synchronization per thread/////////////////////////////////////////////////////////////////////////
double mutextrialperthread(THREADID thr_id)
{
  double ct=0;
    map<THREADID,list<struct accesslocks> >::iterator it;
    

    for(it=mp.begin();it!=mp.end();it++)
    {
      list<struct accesslocks>::iterator lk;

        if(it->first==thr_id)
        {
              for(lk=it->second.begin();lk!=it->second.end();lk++)
             {
                    if(lk->flag==1)
                      ct+=lk->count;
             }
...
double semtrialperthread(THREADID thr_id)
{
  int ct=0;
...
double rw_trialperthread(THREADID thr_id)
{
  double ct=0;
    map<THREADID,list<struct accesslocks> >::iterator it;
...
double barr_trialperthread(THREADID thr_id)
{
  double ct=0;
    map<THREADID,list<struct accesslocks> >::iterator it;
...
double interthreadsim(THREADID thr_id)
{
	std::list<struct sequentialthreadcount>::iterator sq;
	int x=0.0;
	for(sq=sequentialthreadlist.begin();sq!=sequentialthreadlist.end();sq++)
	{
		//cout<<"thread id:"<<sq->tid<<"  sequential_feature:"<<sq->sequence<<"    thread_feature:"<<sq->thread<<"\n";
		if(sq->tid==thr_id)
		{
			if((sq->thread)==0)
			{
...
double locality(THREADID thr_id)
{
	std::list<struct sequentialthreadcount>::iterator sq;
	double x=0.0;;
	for(sq=sequentialthreadlist.begin();sq!=sequentialthreadlist.end();sq++)
...
//sequential and thread count
VOID thread_sequential()
{
	std::map<THREADID,std::vector<ADDRINT> >::iterator it;
	std::map<THREADID,std::vector<ADDRINT> >::iterator it1;
	std::map<THREADID,std::vector<ADDRINT> >::iterator it2;
	int sequence_size,i,count;
	THREADID id1;
	THREADID id2;
	std::vector<ADDRINT> globvector1;
	std::vector<ADDRINT> globvector2;
	for(it=globalvarthreadmap.begin();it!=globalvarthreadmap.end();it++)
	{
		id1=it->first;
		globvector1=it->second;
		//cout<<globvector1.size()<<"\n";
		it2=it;
		it2++;
		std::set<vector<ADDRINT> >unique_sequence;
		std::set<THREADID>thread_sequence;
		//if(it2!=globalvarthreadmap.end() and (globvector1.size())>=2)
		{	
			sequence_size=2;
			int n=globvector1.size();
			while(sequence_size<=n)
			{
			
				for(it1=globalvarthreadmap.begin();it1!=globalvarthreadmap.end();it1++)
				{
					id2=it1->first;
					if(id1!=id2)
					{
						globvector2=it1->second;
						int m=globvector2.size();
						//cout<<m<<"\n";
						count=0;
						if(sequence_size<=m)
						{
							int start1=0;
							int end1=sequence_size-1;
							while(end1<n)
							{
								std::vector<ADDRINT> temp1;
								i=start1;
								while(i<=end1)
								{
									
									temp1.push_back(globvector1[i]);
									i++;
								}
								int start2=0;
								int end2=sequence_size-1;
								while(end2<m)
								{
									std::vector<ADDRINT> temp2;
									i=start2;
									while(i<=end2)
									{
										
										temp2.push_back(globvector2[i]);
										i++;
									}
									i=0;
									int flag=0;
									...
									if(flag==0)
									{
										unique_sequence.insert(temp1);
										thread_sequence.insert(id2);
										count++;
									}
						...
						if(count>0)
						{
							struct sequential temp;
							temp.tid1=id1;
							temp.tid2=id2;
							temp.result=count;
							sequential_list.push_back(temp);
		...
		cout<<"unique_sequence:"<<unique_sequence.size()<<"  thread_sequence :"<<thread_sequence.size()<<"\n";
		struct sequentialthreadcount temp;
		temp.tid=id1;
		temp.sequence=unique_sequence.size();
		temp.thread=thread_sequence.size();
		sequentialthreadlist.push_back(temp);
	}
...
cout<<"Thread SRIJONI"<<lit->tid <<" "<<tr.flag<<" "<<lit1->mutex_var <<"  "<<'\n';
...
littgrmu->flag=1;
cout<<"Thread SRIJONI"<<lit->tid <<" "<<littgrmu->flag<<" "<<lit1->mutex_var <<"  "<<'\n';
...
//cout<<"Thread SRIJONI"<<lit->tid <<" "<<lit->flag <<" "<<lit1->mutex_var <<"  "<<'\n';
struct mutexregion tr;
tr.tid=lit->tid;
tr.flag=9;
tr.addr=lit1->mutex_var;
...
if(litgrmu->tid==lit->tid && (litgrmu->flag==1 || litgrmu->flag==2))
{

cout << " I AM INNNNNN"  << "TID"  << litgrmu->tid << endl;
...
if(litgrmu->tid==lit->tid && (litgrmu->flag==6 || litgrmu->flag==5))
{
...
if(litgrmu->tid==lit->tid && (litgrmu->flag==8 || litgrmu->flag==9))
{
 tr.mutex_var=litgrmu->addr;
  tr.sync="RW_LOCK";
...
else if(litgrmu->tid==lit->tid && litgrmu->flag==11)
{
 tr.mutex_var=litgrmu->addr;
  tr.sync="BARRIER";
tr.type="AFTER";
...
if(lit->order=="BEFORE" && lit->cont=="Reads from variable whose address is" and lit->node_type==0)
{
	...
	/* std::map<ADDRINT,std::list<struct mutexregion> >::iterator iit;
        std::list<struct mutexregion>::iterator it;
        iit=globalmutmap.find(lit1->global_var);
               
        if(iit!=globalmutmap.end())
        {
          //cout<<"addrrrrrrrrr"<<iit->first<< " THREAF ID" <<lit->tid<< endl;
          for(it=iit->second.begin();it!=iit->second.end();it++)
          {
                 struct accesslocks al;
            
                 if(it->flag==0||it->flag==1||it->flag==2)
                      al.flag=1;
                  else if(it->flag==4||it->flag==5||it->flag==6) 
                     al.flag=2;
                  else if(it->flag==7||it->flag==8||it->flag==9) 
                     al.flag=3;
                   else if(it->flag==11) 
                     al.flag=4;

                 if(mp.find(lit->tid)==mp.end())
                  {
                    list<struct accesslocks> lk;
                    al.count=1;
                    al.addr=it->addr;
                    lk.push_back(al);
                    mp[lit->tid]=lk;
                ...
                else
                {
                  int flg=0;
                      ...
                      for(list<struct accesslocks>::iterator i=mp[lit->tid].begin();i!=mp[lit->tid].end();i++)
                      {
                              if(it->addr==i->addr)
                              {
                                flg=1;
                      ...
                      if(flg==0)
                      {
                        al.addr=it->addr;
                        al.count=1;
                        mp[lit->tid].push_back(al);
                      }
                }

          }
          
          //cout<<"MYYYYYYYYYYYYYYYYY"<<mp.size()<<endl;
        }*/
...
std::map<int,std::list<struct globalregion> >::iterator itglob;
for(itglob=globalmap.begin();itglob!=globalmap.end();++itglob)
{
	//cout<<"gmap welcome:"<<itglob->first<<"\n";
	std::list<struct globalregion>::iterator it;
				
			for(it=itglob->second.begin();it!=itglob->second.end(); it++)
			{
				
				cout<<(it->tid)<<" "<<(it->global_var)<<" ";			
				if(globalvarthreadmap.find(it->tid)==globalvarthreadmap.end())
				{
					std::vector< ADDRINT> glb;
					glb.push_back(it->global_var);
					globalvarthreadmap[it->tid]=glb;
				}
				else
				{
					globalvarthreadmap[it->tid].push_back(it->global_var);
				}
...
//std::map<int,std::list<struct globalregion> >::iterator itglob;
std::map<THREADID,std::vector<ADDRINT> >::iterator gvar;
for(gvar=globalvarthreadmap.begin();gvar!=globalvarthreadmap.end();gvar++)
{
	cout<<"THREAD ID :"<<gvar->first<<"   ";
	vector<ADDRINT> arr=gvar->second;
	int n=arr.size();
	int i=0;
	while(i<n)
	{
...
myfile.open ("thread_conc_l.csv",std::fstream::in|std::fstream::out|std::fstream::app);

//for(int i=0;i<NTHREADS;i++){
  //myfile<<"Thread"<<i+1<<"_create"<<","<<"total_create"<<","<<"Thread"<<i+1<<"_signal"<<","<<"Thread"<<i+1<<"_cond"<<","<<"Thread"<<i+1<<"_global"<<","<<"totalglobal"<<","<<"Thread"<<i+1<<"_start"<<","<<"Thread"<<i+1<<"_exit"<<","<<"Thread"<<i+1<<"_join"<<",";
  //}
  //myfile<<"CLASS"<<",";
  //myfile<<"\n";
      ...
      myfile << dataarray[i][0] <<"," << dataarray[i][1] << "," << dataarray[i][2] << "," << dataarray[i][3] << "," << dataarray[i][4] << "," << dataarray[i][5] << "," << dataarray[i][6] << "," << dataarray[i][7] << "," << dataarray[i][8] << ",";
        ...
        if(dataarray[0][0]!=dataarray[0][1]&&dataarray[0][8]==0.05)
            {
            	myfile<<"PIPELINE"<<",";
            ...
            {
            	if(dataarray[0][3]==0.05&&dataarray[0][4]==0.05&&dataarray[0][8]!=0.05)
            	  myfile <<"BOSS WORKER"<<",";
            	else if((dataarray[0][3]!=0.05||dataarray[0][4]!=0.05)&&dataarray[0][8]!=0.05)
            		myfile<<"PEER TO PEER"<<",";
            	else
            		myfile<<"HYBRID"<<",";
            }
...
countuniqueaddr();
set<ADDRINT>::iterator iit;

/////////DATAARRAY2/////////////////////////////////////////////////////////////////////////////////////////
/*
for(iit=uniq_rw.begin();iit!=uniq_rw.end();iit++)
  cout<<"addr rw_lock   "<<*iit<<endl;
cout<<"rwww "<<count_uniq_rw()<<endl;
...
for(int j=0;j< thc;j++)
    ...
    for(int z=0; z< 1; z++)
   {
		...
		std::advance (itth,j);
          ...
          dataarray2[j][z] =global_updates_thread(*itth);
         // cout << t << '\n';
       cout << "global updates thread"  <<dataarray2[j][z] << '\n';
          //itcr++; 
     }
  ...
  for(int j=0;j< NTHREADS;j++)
   {
    for(int z=1; z< 2; z++)
   {
          dataarray2[j][z] = count_globals();
         // cout << t << '\n';
       cout << "TOTAL GLOBAL"  <<dataarray2[j][z] << '\n';
          //itcr++; 
     }

  }
cout<<"thc1111"<<thc<<endl;
//all other features
vector<double> synctotal(thc+1,0);
...
for(int j=0;j<thc;j++)
{
	...
	std::advance (itth,j);
			...
			dataarray2[j][z]=Mutexcountperthread(*itth);
      synctotal[j]+=dataarray2[j][z]; //cal total sync access;

		cout<<"MUTEX global access "<<dataarray2[j][z]<<endl;
	}
}
...
for(int j=0;j<thc;j++)
{
	...
	std::advance (itth,j);
			...
			dataarray2[j][z]=semcountperthread(*itth);
        synctotal[j]+=dataarray2[j][z]; //cal total sync access;
		cout<<"SEM global access"  <<dataarray2[j][z]<<endl;
	}
}
...
for(int j=0;j<thc;j++)
{
    ...
    itth=thlist.begin();
    std::advance (itth,j);
		    dataarray2[j][z]=rwlockperthread(*itth);
          synctotal[j]+=dataarray2[j][z]; //cal total sync access;

		cout<<"READ WRITE global access "<<dataarray2[j][z]<<endl;
	}
}
...
for(int j=0;j<thc;j++)
{
	for(int z=5;z<6;z++)
	{
		itth=thlist.begin();
    std::advance (itth,j);
			dataarray2[j][z]=barcountperthread(*itth);
		
        synctotal[j]+=dataarray2[j][z]; //cal total sync access;
		cout<<"BARRIER access  "<<dataarray2[j][z]<<endl;
	}
}
...
for(int j=0;j<thc;j++)
{
	for(int z=6;z<7;z++)
	{
		itth=thlist.begin();
    std::advance (itth,j);
		dataarray2[j][z]=unsynccountperthread(*itth);
  synctotal[j]+=dataarray2[j][z]; //cal total sync access;
		cout<<"UNSYNC global access  "<<dataarray2[j][z]<<endl;
	}
}
	
myfile.open ("thread_safety_l.csv",std::fstream::in|std::fstream::out|std::fstream::app);

//for(int i=0;i<NTHREADS;i++){
  //myfile<<"Thread"<<i+1<<"_global"<<","<<"total_global"<<","<<"Thread"<<i+1<<"_mutex_syn"<<","<<"Thread"<<i+1<<"_sem_sync"<<","<<"Thread"<<i+1<<"_rw_syn"<<","<<"Thread"<<i+1<<"_barrier_sync"<<","<<"Thread"<<i+1<<"_unsync"<<",";
  //}
  //myfile<<"CLASS"<<",";
  //myfile<<"\n";
...
int flag=0;
      ...
      for (int i = 0; i < NTHREADS; i++){
      myfile << dataarray2[i][0] <<"," << dataarray2[i][1] << "," << dataarray2[i][2] << "," << dataarray2[i][3] << "," << dataarray2[i][4] << "," << dataarray2[i][5] << "," << dataarray2[i][6] << ",";
          
          if(dataarray2[i][6]==0.05&&i<thc)
            flag++;
      }
...
myfile<<count_uniq_mut()<<","<<count_uniq_barr()<<","<<count_uniq_sem()<<","<<count_uniq_rw()<<",";
...
for(int j=0;j< thc;j++)
    ...
    for(int z=0; z< 1; z++)
   {
   		itth=thlist.begin();
		std::advance (itth,j);
        dataarray4[j][z] = global_updates_thread(*itth);
          //dataarray4[j][z] =global_updates_thread(*itth);
         // cout << t << '\n';
       cout << "global"  <<dataarray4[j][z] << '\n';
          //itcr++; 
     }
...
for(int j=0;j<thc;j++)
{
	...
	double x=Mutexcountperthread(*itth),ans=0;

	if(x!=0.05)
	ans+=x;
	
	x=semcountperthread(*itth);
	if(x!=0.05)
	ans+=x;

	x=rwlockperthread(*itth);
	if(x!=0.05)
	ans+=x;

	x=barcountperthread(*itth);
	if(x!=0.05)
	ans+=x;
	...
	if(ans==0.00)
		...
		cout<<"Thread sync access"<<dataarray4[j][z]<<endl;
	}
}
...
for(int j=0;j<thc;j++)
{
    ...
    for(int z=3;z<4;z++)
	{
		...
		dataarray4[j][z]=unsynccountperthread(*itth);

		cout<<"UNSYNC global access"<<dataarray4[j][z]<<endl;
	}
}
...
for(int j=0;j<thc;j++)
{
    ...
    itth=thlist.begin();
    std::advance (itth,j);
		    dataarray4[j][z]=interthreadsim(*itth);

		cout<<"inter thread similerity"<<dataarray4[j][z]<<endl;
	}
}

//cout<<"4444444444444444444444"<<thc<<endl;
for(int j=0;j<thc;j++)
{
	for(int z=5;z<6;z++)
	{
		itth=thlist.begin();
    		std::advance (itth,j);
		dataarray4[j][z]=locality(*itth);
		

		cout<<"Locality_weight"<<dataarray4[j][z]<<endl;
	}
}


std::fstream fs;
fs.open ("data_acces_l.csv",std::fstream::in|std::fstream::out|std::fstream::app);

//for(int i=0;i<NTHREADS;i++)
  //fs<<"Thread"<<i+1<<"_Global"<<","<<"Total_Global"<<","<<"Thread"<<i+1<<"_sync_acces"<<","<<"Thread"<<i+1<<"_UnSync_access"<<","<<"Thread"<<i+1<<"_Inter_thread_simm"<<","<<"Thread"<<i+1<<"_Locality_weight"<<",";
//fs<<"Class";
  
  //fs<<"\n";
   	for (int i = 0; i < NTHREADS; i++){
      	fs << dataarray4[i][0] <<"," << dataarray4[i][1] << "," << dataarray4[i][2] << "," << dataarray4[i][3] << "," << dataarray4[i][4] << "," << dataarray4[i][5]<<",";}
int flag1=1,flag2=1;
for(int i=0;i<thc;i++)
{
	if(dataarray4[i][4]<=(thc/2) or dataarray4[i][5]<dataarray4[i][1])
	{
		flag1=0;
	}
	if(dataarray4[i][3]!=0.05)
	{
		flag2=0;
	}
...
{
cout << "Displaying count Map " <<'\n';


for(itgrcu = countmap.begin(); itgrcu!= countmap.end(); ++itgrcu)
{

cout << " First Element of count Map " << '\n' << itgrcu->first << '\n';

cout <<" ----- The second element lists are -----"<<'\n';               
std::list<struct countmutexregion>::iterator count_2nd_itr;
               
for(count_2nd_itr=itgrcu->second.begin(); count_2nd_itr!=itgrcu->second.end(); ++count_2nd_itr)
{

cout<<"tid "<<count_2nd_itr->tid<<'\n';
cout<<"count "<<count_2nd_itr->count<<'\n';
...
cout << " First Element of global Map " << '\n' << itgr->first << '\n';

cout <<" ----- The seglobal element lists are -----"<<'\n';               
std::list<struct globalregion>::iterator global_2nd_itr;
               
for(global_2nd_itr=itgr->second.begin(); global_2nd_itr!=itgr->second.end(); ++global_2nd_itr)
{

cout<<"Thread "<<global_2nd_itr->tid<<'\n';
cout<<"global_variable "<<global_2nd_itr->global_var<<'\n';
cout<<"mutex_variable "<<global_2nd_itr->mutex_var<<'\n';
cout<<"sem_variable "<<global_2nd_itr->sem_var<<'\n';
cout<<"rw_variable "<<global_2nd_itr->rw_var<<'\n';
cout<<"sync "<<global_2nd_itr->sync<<'\n';
cout<<"type "<<global_2nd_itr->type<<'\n';
}
...
list<ADDRINT>::iterator xxx;

cout<<"GGGGGGGGGGGGGGLLLLLLLLLLIST \n";
for(xxx=glist.begin();xxx!=glist.end();xxx++)
cout<<*xxx<<" ";
