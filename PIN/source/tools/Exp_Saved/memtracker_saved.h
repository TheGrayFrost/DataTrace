...
extern stack <int> stacks;						// current stack structure
...
extern map <ADDRINT, int> globals;				// global address -> variable index
extern map <pair<ADDRINT, int>, int> locals; 	// local address + stack no. -> variable index
extern int stno, countG, countL;
// reduce memory requirements later
...
int getGlobal (ADDRINT memOp)
{
	if (globals.find(memOp) == globals.end())
		globals[memOp] = ++countG;
	return globals[memOp];
}

int getLocal (ADDRINT memOp)
{
	auto k = make_pair(memOp, stacks.top());
...
//to be removed later
	...
	VARINFO() {};
	VARINFO(string rA, ADDRINT aA, int st, bool g)
	{
		relAdd = rA; absAdd = aA; stloc = st;
		if (g) varname = "G_" + to_string(getGlobal(aA));
		else varname = "L_" + to_string(getLocal(aA));
	}
...
struct INSINFO
{
	...
	set <string> regR, regW;	// registers read from and written to
	...
	set <string> memB;			// memory register break
...
INSINFO::INSINFO(INS& ins)
{
	...
	{
		regR.insert(REG_StringShort(INS_RegR(ins, i)));
		if (REG_is_seg(INS_RegR(ins, i)))
			memR = memW = false;
	}
	for (int i = 0; i < wreg; ++i)
	{
		regW.insert(REG_StringShort(INS_RegW(ins, i)));
