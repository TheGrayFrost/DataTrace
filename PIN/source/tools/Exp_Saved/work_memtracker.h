...
// the complete address is kept in mem
string extracter (string& diss, string& mem)
{
	auto k = parse(diss);
	int idx = 1;
	if (k[2].find("ptr") != -1)
	...
	string u;
	int l = dpart.find('[');
	mem = dpart.substr(l+1, dpart.length()-l-2);
...
struct INSINFO
{
	...
	string extract;				// relative memory extract
	string mem;					// exact memory string
	string op2;					// register written from in case of memory write (?)
